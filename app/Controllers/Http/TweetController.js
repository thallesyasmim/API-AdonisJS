...
class TweetController {
  ...
  async store ({ request, auth }) {
    try {
      ...
      const tweet = await Tweet.create({ user_id: auth.user.id, content }) // Criando um Tweet do usuário que está autenticado
      return tweet
    } catch (error) {
  ...
  async destroy ({ params, auth, response }) {
    ...
    if(tweet.user_id !== auth.user.id) { // Duas validações: 1. Usuário autenticado; 2. Tweet é dele?
